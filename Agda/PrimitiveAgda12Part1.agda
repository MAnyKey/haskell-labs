...
lemma-*zero zero = refl
lemma-*zero (succ y) = lemma-*zero y
...
-- lemma-*succ : ∀ a b → a + a * b ≡ a * (succ zero + b)
lemma-*succ zero b = refl
lemma-*succ (succ y) b = cong succ ((lemma-+swap y b (y * b)) ~ (cong (λ x → b + x) (lemma-*succ y b)))

*-comm : ∀ a b → a * b ≡ b * a
*-comm zero b = lemma-*zero b
*-comm (succ y) b = (cong (λ x → b + x) (*-comm y b)) ~ (lemma-*succ b y)
